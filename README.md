...
The file names are editable in the Layers tab. While the bridge effect is in the
chain, the app watches that output file and composites it over the prompted
...
| `fx_core.py` | all effects, including the two new AI layer effects |
| `fx_phase_rail.py` | reusable CUDA/torch PhaseRail engine |
| `selftest.py` | tests every effect and preset without a camera |
